...
public void getAnotherDeviceFromWeb() throws Exception {
			...
			wait(getDriver(), 20).until(ExpectedConditions
			...
			Select s = new Select(wait(getDriver(), 20).until(
			...
			String actualcost = wait(getDriver(), 20).until(
			...
			boolean nextpage = wait(getDriver(), 20)
					.until(ExpectedConditions
			...
			// Enter city
			wait(getDriver(), 20).until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//input[@placeholder='City']")))
					.sendKeys(City);
	
			// Select state
			Select state = new Select(wait(getDriver(), 20)
			...
			boolean nextpage2 = wait(getDriver(), 20)
					.until(ExpectedConditions
			...
			String A_address = wait(getDriver(), 20)
					.until(ExpectedConditions
							.visibilityOfElementLocated(By.xpath("//input[@placeholder='Street Address']")))
					.getAttribute("value");
			...
			String A_City = wait(getDriver(), 20)
			...
			String A_state = wait(getDriver(), 20)
					.until(ExpectedConditions
			...
			boolean nextpage3 = wait(getDriver(), 20)
					.until(ExpectedConditions
			...
			// Enter card CVV
			wait(getDriver(), 20).until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//input[@placeholder='CVV']")))
					.sendKeys(Card_CVV);
	
			// Select card month
	
			Select month = new Select(wait(getDriver(), 20)
					.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//select[@placeholder='Month']"))));
			month.selectByVisibleText("12");
	
			// Select card year
			Select year = new Select(wait(getDriver(), 20)
			...
			if(order_placed==false) 
			{
				try 
				{
					order_placed=	wait(getDriver(), 30).until(ExpectedConditions.visibilityOfAllElementsLocatedBy(By.xpath("//h2[contains(text(),'Thank You For Your Order')]"))).size() == 1;
				} 
				catch(Exception e) 
				{
			...
			String order_des = wait(getDriver(), 30).until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//p[@class='confirm-row'][2]"))).getText();
			...
			// Sub-total
			String Act_sub_total = wait(getDriver(), 20).until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//li[@class='amount-container'][1]/div[2]"))).getText();				
			softassert.assertEquals(Act_sub_total, "$59.94");
			
			
			// Verify Free Trial Period
			String Act_free_trial = wait(getDriver(), 20).until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//li[@class='amount-container'][2]/div[2]"))).getText();				
			softassert.assertEquals(Act_free_trial, "30 Days");
			
			// Verify Shipping
			String Act_shipping = wait(getDriver(), 20).until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//li[@class='amount-container'][3]/div[2]"))).getText();				
			softassert.assertEquals(Act_shipping, "Free");
			
			
			// Verify Tax
			String Act_tax = wait(getDriver(), 20).until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//li[@class='amount-container'][4]/div[2]"))).getText();				
			softassert.assertEquals(Act_tax, "$0.00");
			
			// Verify Total
			String Act_total = wait(getDriver(), 20).until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//li[@class='amount-container total']/div[2]"))).getText();				
			softassert.assertEquals(Act_total, "$59.94");
			
			
			// Verify Your Shipping Information
			// Verify user-name
			String Act_username = wait(getDriver(), 20).until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//ul[@class='order-list top']/li[2]"))).getText();				
			softassert.assertEquals(Act_username, "Test Demo");
			
			//Verify Email
			String Act_email = wait(getDriver(), 20).until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//ul[@class='order-list top']/li[3]"))).getText();				
			softassert.assertEquals(Act_email, "Demo@gmail.com");
			
			
			//Verify Address
			String Act_address = wait(getDriver(), 20).until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//ul[@class='order-list top']/li[4]"))).getText();				
			...
			// Verify card name
			String Act_card_name = wait(getDriver(), 20).until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//ul[@class='order-list']/li[2]"))).getText();				
			softassert.assertEquals(Act_card_name, "Demo card");
			
			// Verify card number
			String Act_card_no = wait(getDriver(), 20).until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//ul[@class='order-list']/li[3]"))).getText();				
			softassert.assertEquals(Act_card_no, "**** **** **** 4242");
			
			// Verify phone number
			String Act_phone_no = wait(getDriver(), 20).until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//ul[@class='order-list']/li[4]"))).getText();				
			softassert.assertEquals(Act_phone_no, "1236547895");
			
			//Scrolling page to bottom
			JavascriptExecutor js = (JavascriptExecutor) getDriver();  
			WebElement ele = wait(getDriver(), 20).until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//h3[contains(text(),'Share Your')]")));
			js.executeScript("arguments[0].scrollIntoView();", ele);
			Thread.sleep(1000);	
			
			//Verify Your Promo Code
			String act_promo_code = wait(getDriver(), 20).until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//h3[contains(text(),'Your Promo Code')][2]/following-sibling::h2"))).getText();
