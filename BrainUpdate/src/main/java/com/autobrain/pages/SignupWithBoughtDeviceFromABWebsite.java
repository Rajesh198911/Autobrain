...
public class SignupWithBoughtDeviceFromABWebsite extends Base {
	...
	public SignupWithBoughtDeviceFromABWebsite(SignupModel object) {
		// Initializing
		login = new Login();
		ref = PageFactory.initElements(getDriver(), SignupModel.class);
		obj = object;
		obj.setAll_Devices_No(new ArrayList<String>());
	}
	...
	public void signupWithBoughtDeviceFromABWebsite() throws Exception {
		...
		step1Setup(obj.getAll_Devices_No().get(0));
		...
		if (obj.getAll_Devices_No().size() > 1) {

			for (int i = 1; i < obj.getAll_Devices_No().size(); i++) {
				activateNewDevice(obj.getAll_Devices_No().get(i));
			}
		}
	...
	public void signup() throws Exception {
		...
		if ((obj.getOwner_email() == null)) {
			obj.setOwner_email(GenerateRandomEmail());
		}
		...
		VisibilityOfElementByID("user_last_name", 15).sendKeys("" + obj.getRandom_int());

		// Entering email
		VisibilityOfElementByID("user_email", 15).clear();

		// Sign-up email
		VisibilityOfElementByID("user_email", 15).sendKeys(obj.getOwner_email());
		...
		for (int i = 0; i < getDriver().findElements(By.xpath("//span[@class='help-block']")).size(); i++) {
			...
			if (firstN_alert) {
				VisibilityOfElementByID("user_first_name", 15).clear();
				VisibilityOfElementByID("user_first_name", 15).sendKeys("demouser" + obj.getRandom_int());
			}

			// Last name already exist
			if (lastN_alert) {
				VisibilityOfElementByID("user_last_name", 15).clear();
				VisibilityOfElementByID("user_last_name", 15).sendKeys("Last" + obj.getRandom_int());
			}
			...
			if (email_alert) {
				...
				VisibilityOfElementByID("user_email", 15).clear();
				obj.setOwner_email(GenerateRandomEmail());
				System.out.println("Email Already exist! New Email Is-->" + " " + obj.getOwner_email());
				VisibilityOfElementByID("user_email", 15).sendKeys(obj.getOwner_email());
...
//			System.out.println("Owner email: " + obj.getOwner_email());
//
...
//
	...
	public synchronized void orderDeviceFromWebSite() throws Exception {
		...
		if (obj.getAccount_type().equalsIgnoreCase("personal")) {
			account_type = "Autobrain Family";
		}

		if (obj.getAccount_type().equalsIgnoreCase("business")) {
		...
		s.selectByVisibleText(String.valueOf(obj.getTotal_bought_devices()));
		...
		// Enter email address
		VisibilityOfElementByXpath("//input[@placeholder='Email Address']", 15).sendKeys(obj.getOwner_email());
		System.out.println("Entered Email During Bought The Product " + "--> " + obj.getOwner_email());
		...
		Assert.assertTrue(VisibilityOfAllElementsByXpath("//p[contains(text(),'Your package will include "
				+ obj.getTotal_bought_devices() + " Autobrain device as well as a 5 step quick start guide.')]", 15)
						.size() == 1,
	...
	public void orderToShip() throws Exception {
		...
		for (int i = 0; i < ref.no_of_input_devices.size(); i++) {
			if (obj.getBluetooth_is().equals("upgraded_device")) {
				TypeInFieldSlowly(ref.no_of_input_devices.get(i), obj.getAll_Devices_No().get(1));
			}

			else {
				TypeInFieldSlowly(ref.no_of_input_devices.get(i), obj.getAll_Devices_No().get(i));
			}
			Thread.sleep(3000);
	...
	public void step1Setup(String device_no) throws Exception {
		...
		VisibilityOfElementByXpath(
				"//label[contains(text(),'Create a Name for Your Car')]/following-sibling::div[1]/input", 15)
						.sendKeys(obj.getAll_Devices_No().get(0));
		...
		if (!(obj.getBluetooth_is().contains("free"))) {
			if (!(obj.isSet_esf())) {
	...
	public void choosePricingPlanAndAddCardDetails() throws Exception {
		...
		// FAMILY PLAN
		if (obj.getAccount_type().equals("personal")) {

			// Choose Plan
			WebElement choose_plan = VisibilityOfElementByXpath(obj.getPersonal_plan(), 15);
			choose_plan.click();

			// Choose Billing Interval
			WebElement duration = VisibilityOfElementByXpath(obj.getPersonal_billing_interval(), 15);
			duration.click();
			Thread.sleep(4000);
		}

		// BUSINESS PLAN
		if (obj.getAccount_type().equals("business")) {

			// Choose Billing Interval
			WebElement billing_interval = PresenceOfElementByXpath(obj.getChoose_business_billing_interval(), 15);
			billing_interval.click();
			Thread.sleep(2000);
		}

		if (obj.getBluetooth_is().equals("free")) {
...
//		v.Ins_Reg_Forms();
		...
		if (obj.getAccount_type().contains("personal")) {
			...
			if (obj.getPersonal_plan().contains("//div[@class='_1nPLChEwNgDH5KMyzoXBEb_0']/div[3]//button")) {
				num = 0;
			}
			if (obj.getPersonal_plan().contains("//div[@class='_1nPLChEwNgDH5KMyzoXBEb_0']/div[2]//button")) {
				num = 1;
			}
			if (obj.getPersonal_plan().contains("//div[@class='_1nPLChEwNgDH5KMyzoXBEb_0']/div[1]//button")) {
				num = 2;
			}
	...
	public ArrayList<String> createDeviceFromPanel() throws Exception {
		...
		for (int i = 0; i < obj.getTotal_bought_devices(); i++) {
	...
	public void EsfExemptionsSetup() throws Exception {
		...
		if (obj.isSet_esf()) {
			...
			// Enter email
			VisibilityOfElementByXpath("//input[@id='email']", 15).sendKeys(obj.getOwner_email());
			...
			try {
				email_added = VisibilityOfAllElementsByXpath("//td[contains(text(),'" + obj.getOwner_email() + "')]",
						15).size() == 1;
			} catch (Exception e) {
				email_added = false;
			}
	...
	public String getConfirmationCode() throws Exception {
		...
		// Input email
		VisibilityOfElementByXpath("//input[@name='box']", 15).sendKeys(obj.getOwner_email());
		System.out.println("Owner email: " + obj.getOwner_email());
	...
	public String GenerateRandomEmail() {
		...
		obj.setRandom_int(randomGenerator.nextInt(10000));
		String email = "junking" + obj.getRandom_int() + "@mailcatch.com";
		SignupModel.setL_name("" + obj.getRandom_int());
		return email;
	}
