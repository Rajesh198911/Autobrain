...
public class SignupWithRetailerDevice extends Base {
	...
	SignupModel obj;
	String lockRetailer = "lockRetailer";

	public SignupWithRetailerDevice(SignupModel object) {
		login = new Login();
	...
	public void signupWithRetailerDevice() throws Exception {
		...
		if (obj.getAll_Devices_No().size() > 1) {
			...
			for (int i = 1; i < obj.getAll_Devices_No().size(); i++) {
				signup.activateNewDevice(obj.getAll_Devices_No().get(i));
			}
		}
	...
	public void addDevicesInCsvFile() throws Exception {
		...
		for (int i = 0; i < obj.getAll_Devices_No().size(); i++) {
			String number = obj.getAll_Devices_No().get(i);
			String[] num = { number };
	...
	public void CreateInvoice() throws Exception {
		...
		else if (obj.getAccount_type().contains("business")) {
	...
	public void ChooseInvoicePricingPlanAndDistributionChannel() throws Exception {
		...
		choose_pricing_plan.selectByVisibleText(obj.getPricing_plan());
