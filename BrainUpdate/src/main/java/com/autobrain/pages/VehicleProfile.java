...
public class VehicleProfile extends Login {
	...
	String carname2 = "Testing car", caricon2 = "//div[@class='grid']/div[10]",
			status2 = "//div[@class='_3fRlA9ofZ3wg1XWxsL0y2l_0']/parent::div", year2 = "2018", make2 = "VOLVO",
			VIN2 = "MCSJ879373J736G";
	int model2 = 6;
	...
	public void VehicleProfileInfo() throws Exception {
		...
		wait(getDriver(), 15)
				.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[@class='hamburger-container']")))
				.click();
		Thread.sleep(1000);
		// Click on Vehicle Profile from Menu
		wait(getDriver(), 15).until(
				ExpectedConditions.visibilityOfElementLocated(By.xpath("//span[contains(text(),'Vehicle Profile')]")))
				.click();
		...
		// Car Name
		wait(getDriver(), 15).until(
				ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[@class='new-device-form']/div[1]/input")))
				.clear();
		...
		wait(getDriver(), 15)
				.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[@class='select-dropdown']/div")))
				.click();

		Thread.sleep(2000);

		// Selecting Car Icon from drop-down
		wait(getDriver(), 15).until(ExpectedConditions.visibilityOfElementLocated(By.xpath(caricon))).click();

		// Enter VIN number
		wait(getDriver(), 15).until(ExpectedConditions.visibilityOfElementLocated(
				By.xpath("//label[contains(text(),'VIN Number')]/following-sibling::div/input"))).clear();

		wait(getDriver(), 15).until(ExpectedConditions.visibilityOfElementLocated(
				By.xpath("//label[contains(text(),'VIN Number')]/following-sibling::div/input"))).sendKeys(VIN);

		// Is your car hybrid status (ON/OFF)
		wait(getDriver(), 15).until(ExpectedConditions.visibilityOfElementLocated(By.xpath(status))).click();
		Thread.sleep(2000);
		...
		String page_loaded = wait(getDriver(), 40).until(
				ExpectedConditions.visibilityOfElementLocated(By.xpath("//h4[contains(text(),'Vehicle Profile')]")))
				.getText();

		softassert.assertEquals(page_loaded, "Vehicle Profile",
				"Vehicle Profile not loaded after udpate the vehicle form!");

		getDriver().navigate().refresh();

		// Validate page loaded completely after refresh
		String page_loaded_after_refresh = wait(getDriver(), 40).until(
				ExpectedConditions.visibilityOfElementLocated(By.xpath("//h4[contains(text(),'Vehicle Profile')]")))
				.getText();

		softassert.assertEquals(page_loaded_after_refresh, "Vehicle Profile",
				"Vehicle Profile not loaded after refresh the page!");
		Thread.sleep(4000);
	...
	public void Ins_Reg_Forms() throws Exception {
		...
		// License Plate No
		wait(getDriver(), 5).until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//input[@placeholder='TYP013']")))
				.clear();
		wait(getDriver(), 25)
		...
		wait(getDriver(), 25).until(ExpectedConditions.visibilityOfElementLocated(By.xpath(effectivedate))).click();
		...
		wait(getDriver(), 15).until(ExpectedConditions.visibilityOfElementLocated(By.xpath(expiredate))).click();

		// Enter insurance company name
		wait(getDriver(), 5).until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//input[@placeholder='Name']")))
				.clear();
		wait(getDriver(), 5).until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//input[@placeholder='Name']")))
		...
		Thread.sleep(1000);
		Insurance_Eff_Date = wait(getDriver(), 5).until(ExpectedConditions
				.visibilityOfElementLocated(By.xpath("//li[contains(text(),'Effective Date')]/following-sibling::li")))
				.getText();

		Insurance_Exp_Date = wait(getDriver(), 5)
				.until(ExpectedConditions
						.visibilityOfElementLocated(By.xpath("//div[@class='car-insurance-info']//div[3]//li[2]")))
				.getText();
...
//NEW FORM CAR REGISTRATION (Click on Edit button) 
		...
		// Expiration Date (Clicking to open date calendar)
		wait(getDriver(), 5)
				.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//input[@placeholder='Pick A Date']")))
				.click();
		Thread.sleep(1000);
		...
		Thread.sleep(1000);
		car_Reg_exp = wait(getDriver(), 5)
				.until(ExpectedConditions
						.visibilityOfElementLocated(By.xpath("//div[@class='car-registration-info']//div[2]//li[2]")))
				.getText();
	...
	public void validate_data() throws Exception {
		...
		// VALIDATE VIN
		String act2 = wait(getDriver(), 5)
				.until(ExpectedConditions.visibilityOfElementLocated(
						By.xpath("//label[contains(text(),'VIN Number')]/following-sibling::div/input")))
				.getAttribute("value");
		...
		String act7 = wait(getDriver(), 5).until(ExpectedConditions
				.visibilityOfElementLocated(By.xpath("//li[contains(text(),'Effective Date')]/following-sibling::li")))
				.getText();
		softassert.assertEquals(act7, (Insurance_Eff_Date), "Vehicle insurance effective date is not updated!");

		// Validate Expire Date
		String act8 = wait(getDriver(), 5)
				.until(ExpectedConditions
						.visibilityOfElementLocated(By.xpath("//div[@class='car-insurance-info']//div[2]//li[2]")))
				.getText();
		softassert.assertEquals(act8, (Insurance_Exp_Date), "Vehicle insurance expiring date is not updated!");

		// Validate Policy number
		String act9 = wait(getDriver(), 5).until(ExpectedConditions
				.visibilityOfElementLocated(By.xpath("//li[contains(text(),'Policy Number')]/following-sibling::li")))
				.getText();
		softassert.assertEquals(act9, policyno, "Vehicle policy no. is not updated!");

		// Validate Insurance company name
		String act10 = wait(getDriver(), 5).until(ExpectedConditions.visibilityOfElementLocated(
				By.xpath("//li[contains(text(),'Insurance Company')]/following-sibling::li"))).getText();
		...
		String act11 = wait(getDriver(), 5)
				.until(ExpectedConditions
						.visibilityOfElementLocated(By.xpath("//div[@class='car-registration-info']//div[1]//li[2]")))
				.getText();
